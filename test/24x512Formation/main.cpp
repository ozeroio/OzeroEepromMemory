#include <Arduino.h>
#include <External24x512Eeprom/External24x512Eeprom.cpp>
#include <ExternalEeprom/ExternalEeprom.cpp>
#include <ExternalEepromFormation/ExternalEepromFormation.cpp>
#include <Wire.h>
#ifdef ARDUINO_ARCH_ESP32
#include <freertos/FreeRTOS.h>
#define LEN (65536 - 1)// -1 so we don't exceed array size limit
#else
#define LEN 1024
#endif

External24x512Eeprom eeprom;
...
void setup() {
	...
	if (match) {
		Serial.println("PASSED the first half!");
	}
	...
	if (match) {
		Serial.println("PASSED second half!");
	}
	free(data);
	free(read);
}
